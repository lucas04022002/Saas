...
export function BookTable({ books, reference_book, favourite }: { books: Book[]; reference_book: MatchDetail["reference_book"]; favourite: Outcome | null }) {
  return (
    <table className="mt-8 w-full border-collapse text-[16px]">
      <caption className="sr-only">Cotes par bookmaker</caption>
      <thead><tr>{["Book", "1", "N", "2", "Marge"].map((h, i) => <th key={h} scope="col" className={`border-b border-ink pb-3 text-[12px] font-semibold uppercase tracking-[0.06em] text-muted ${i ? "text-right" : "text-left"}`}>{h}</th>)}</tr></thead>
      <tbody>
