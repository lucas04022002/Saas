  ...
  return (
    <section className="bg-black text-paper">
      <div className="site py-16 md:py-24">
        <p className="eyebrow text-faint-text">Track record · public</p>
        ...
        {sel === null ? <p className="hair-dark mt-8 py-8 text-[19px]">Pas encore de match terminé avec un relevé de cotes. Le track record commence au premier coup d&apos;envoi.</p> : (
          <div className="mt-4 grid gap-12 md:grid-cols-2 md:gap-16">
            <div>
              ...
              <p className="mt-6 max-w-[40ch] text-[19px] leading-relaxed text-faint-text">des favoris ont gagné en {COMPETITIONS[sel.competition] ?? sel.competition} cette saison, sur {sel.played} matchs. {note} Le favori affiché est celui du dernier relevé avant le coup d&apos;envoi, jamais recalculé après coup.</p>
            </div>
            <table className="w-full border-collapse text-[16px]">
              <caption className="sr-only">Track record par compétition</caption>
              <thead><tr>{["Compétition", "Matchs", "Favori gagnant", "Taux"].map((h, i) => <th key={h} scope="col" className={`border-b border-paper pb-3 text-[12px] font-semibold uppercase tracking-[0.06em] text-faint-text ${i ? "text-right" : "text-left"}`}>{h}</th>)}</tr></thead>
              <tbody>{items.map((r) => (
